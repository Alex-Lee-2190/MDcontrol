...
std::string g_MasterTargetBasePath = ""; 

void SetMasterDownloadTarget() {
    g_MasterTargetBasePath = SystemUtils::GetCurrentExplorerPath();
...
void MasterSendFileThread(std::shared_ptr<SlaveCtx> ctx, uint32_t taskId, std::vector<FileClipInfo> localRoots) {
    ...
    auto sendPkt = [&](const char* data, int len) -> bool {
        while (g_Running && !task->cancelled) {
            ...
            if (targetSock == ctx->fileSock && ctx->fileSock != INVALID_SOCKET_HANDLE) {
                ctx->tcpFileFailed = true;
                if (ctx->fileSock != ctx->btFileSock) {
...
extern std::map<uint32_t, RelayTaskInfo> g_RelayBySourceId; 
extern std::map<std::pair<void*, uint32_t>, uint32_t> g_RelayByDestId; 
...
void FileReceiverThreadFunc(std::shared_ptr<SlaveCtx> ctx) {
    ...
    while (g_Running && ctx->connected && mySock != INVALID_SOCKET_HANDLE) {
            ...
            if (flag == 13) { 
                ...
                bool isRelay = false;
                {
                    ...
                    if (itDst != g_RelayByDestId.end()) {
                        ...
                        if (fwd && fwd->fileSock != INVALID_SOCKET_HANDLE) {
                ...
                if (isRelay) { processed += 17; continue; }
                    ...
                    if (!sourcePath.empty()) {
                        ...
                        if (isDir) {
                            ...
                            while (!task->cancelled && task->senderSession == mySession) {
                                file.open(SystemUtils::Utf8ToWString(sourcePath).c_str(), std::ios::in | std::ios::binary);
                                if (file.is_open()) {
                            ...
                            if (fileOpened) {
                                ...
                                while (true) {
                                    ...
                                    if (isBt) {
                                        uint32_t currentLatency = ctx->latency.load();
                        ...
                        if (task->senderSession == mySession) {
                            if (fileIdx == (int)task->serverTransferList.size() - 1 || task->cancelled) {
                                if (!task->cancelled) {
                                    ...
                                    std::string exeDir = SystemUtils::GetAppDir();
                                    ...
                                    std::thread([testFile](){
                                        SystemUtils::LaunchHashTest(testFile);
                                    }).detach();
                                }
                ...
                if (isRelay) { processed += tempOffset; continue; }
                ...
                if (task) {
                    ...
                    std::thread([task, ctx, taskId, infos]() {
                        ...
                        task->startReceiverTransferCallback = [task, ctx, taskId]() {
                            ...
                            if (task->totalFilesToReceive > 0) {
                                ...
                                auto localSendPkt = [task, ctx, taskId](const char* data, int len) -> bool {
                                    while (g_Running && !task->cancelled) {
                                        SocketHandle targetSock = ctx->fileSock;
                                        if (targetSock == INVALID_SOCKET_HANDLE) return false;
                ...
                if (isRelay) { processed += reqLen; continue; }
                ...
                if (isRelay) { processed += 21 + chunkSize; continue; }
                    ...
                    if (task && !task->cancelled) {
                        if (task->receivingFiles.find(fileIdx) == task->receivingFiles.end()) {
                            ...
                            if (!fullPath.empty()) {
                                ...
                                task->receivingFiles[fileIdx] = std::make_shared<std::ofstream>(SystemUtils::Utf8ToWString(fullPath).c_str(), std::ios::out | std::ios::binary | std::ios::trunc);
                ...
                if (isRelay) { processed += 9; continue; }
                        ...
                        if (!name.empty() && !task->cancelled && !isSkipped) {
                            ...
                            try {
                                if (isDir) {
                                    ...
                                    if (target.has_parent_path()) fs::create_directories(target.parent_path());
                                    std::ofstream ofs(SystemUtils::Utf8ToWString(target.u8string()).c_str(), std::ios::out | std::ios::binary | std::ios::trunc);
                                }
                            } catch (...) {}
                        }
                    }
                    ...
                    if (transferComplete) {
                        ...
                        if (task->filesReceived == task->totalFilesToReceive) {
                            MDC_LOG_DEBUG(LogTag::TRANS, "Trigger receiver hash test on master for task %u", taskId);
                            {
                                ...
                                bool useBt = targetSock != INVALID_SOCKET_HANDLE && SystemUtils::IsBluetoothSocket(targetSock);
                                std::string protocol = useBt ? "BTH" : "TCP";
                                std::string slave_addr = SystemUtils::GetPeerAddress(targetSock);
